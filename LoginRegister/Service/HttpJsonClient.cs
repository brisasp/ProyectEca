...
namespace LoginRegister.Services
{
    internal class HttpJsonService<T> : IHttpJsonProvider<T> where T : class
    {
        ...
        public async Task<T?> GetByIdAsync(string urlTemplate, int id)
        {
            try
            {
                using HttpClient httpClient = new HttpClient();
                ...
                string url = string.Format($"{Constants.BASE_URL}{urlTemplate}", id);
                HttpResponseMessage response = await httpClient.GetAsync(url);

                if (response.StatusCode == System.Net.HttpStatusCode.Unauthorized)
                {
                    await Authenticate(urlTemplate, httpClient, response);
        ...
        public async Task Delete(string urlTemplate, int id)
        {
            try
            {
                using HttpClient httpClient = new HttpClient();
